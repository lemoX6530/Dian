...
void IMUupdate(float gx, float gy, float gz, float ax, float ay, float az)
{
        ...
        float vx, vy, vz;
        float ex, ey, ez;  
        float accelx=ax,accely=ay,accelz=az;
    ...
    if (Pitch == 0) {
        ...
        relative_yaw = atan2(sin_roll, cos_pitch);
        relative_yaw = relative_yaw * 180.0 / M_PI;
    }
    accelx+=G_accel*sin_pitch;
    accely+=G_accel*sin_roll;
    accelz-=G_accel*cos(relative_yaw*M_PI/180);
    char accel_all[100]="";
    char speed_all[100]="";
    char angle_all[100]="";
    speedx+=accelx*0.01;
    speedy+=accely*0.01;
    speedz+=accelz*0.01;
    sprintf(speed_all,"X:%fm/s,Y:%fm/s,Z:%fm/s\n",speedx,speedy,speedz);
